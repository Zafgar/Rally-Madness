...
##
##   godot --headless --fixed-fps 60 --path . res://tests/ai_probe.tscn -- <track_id> <car_id> <skill>
...
func _ready() -> void:
	...
	var skill: float = float(args[2]) if args.size() > 2 else 0.3
	...
	_driver = AIDriver.new(_car, _builder, skill, 0.0)
	...
	print("=== AI probe: %s on %s, skill %.2f ===" % [spec.display_name(), track_id, skill])
...
func _physics_process(delta: float) -> void:
	...
	if _elapsed >= DURATION or _car.damage.wrecked:
		...
		print("  crashes: %d   avg %.1f km/h   max %.1f km/h   condition %.2f" % [
			_impacts, _speed_sum / maxf(_samples, 1), _max_speed, _car.damage.overall()])
		get_tree().quit(0)
